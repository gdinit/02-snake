...
Cell::Cell()
{
	...
	// LOAD TEXTURES
	if ( !m_squareTexture.loadFromFile( "assets/textures/texturepak.png" ) ) {
...
void Cell::newRound()
{
	// CREATE ALL CELLS IN THE DATABASE (cells CONFIG_FIRSTCELL-CONFIG_LASTCELL)
	...
	for ( unsigned short int n = CONFIG_FIRSTCELL; n < CONFIG_LASTCELL; ++n )
	{
	...
	for ( int n = 0; n < CONFIG_COLUMNS; n++ ) {
		m_cellDetailsDatabase.at( n + ( ( CONFIG_ROWS - 1 ) * CONFIG_COLUMNS ) ).cellCurrentContent = 1;
	}

	// LEFT
	for ( int n = 0; n < ( CONFIG_ROWS * CONFIG_COLUMNS ); n += CONFIG_COLUMNS ) {
		m_cellDetailsDatabase.at( n ).cellCurrentContent = 1;
	}

	// RIGHT
	for ( int n = CONFIG_COLUMNS - 1; n < ( CONFIG_ROWS * CONFIG_COLUMNS ); n += CONFIG_COLUMNS ) {
		m_cellDetailsDatabase.at( n ).cellCurrentContent = 1;
	}
...
void Cell::update( sf::Time timeSinceLastUpdate )
{
	...
	// UPDATE CELL SPRITE DATABASE TO ENSURE IT CONTAINS CORRECT SPRITE FOR EACH CELL
	for ( unsigned short int n = CONFIG_FIRSTCELL; n < CONFIG_LASTCELL; ++n ) {
		// TODO switch to switch statement here for better efficiency?
		if ( m_cellDetailsDatabase.at( n ).cellCurrentContent == 0 ) {
			m_cellSpriteDatabase [ n ].setTextureRect( sf::IntRect( 0, 0, 16, 16 ) );// empty=blue
		} else if ( m_cellDetailsDatabase.at( n ).cellCurrentContent == 1 ) {
			m_cellSpriteDatabase [ n ].setTextureRect( sf::IntRect( 48, 0, 16, 16 ) );	// border=red
		} else if ( m_cellDetailsDatabase.at( n ).cellCurrentContent == 2 ) {
			m_cellSpriteDatabase [ n ].setTextureRect( sf::IntRect( 64, 0, 16, 16 ) );	// snake=yellow
		} else if ( m_cellDetailsDatabase.at( n ).cellCurrentContent == 3 ) {
			m_cellSpriteDatabase [ n ].setTextureRect( sf::IntRect( 32, 0, 16, 16 ) );	// fruit=white
		} else if ( m_cellDetailsDatabase.at( n ).cellCurrentContent == 4 ) {
			m_cellSpriteDatabase [ n ].setTextureRect( sf::IntRect( 96, 0, 16, 16 ) );	// obstacle=pink
		} else if ( m_cellDetailsDatabase.at( n ).cellCurrentContent == 5 ) {
			m_cellSpriteDatabase [ n ].setTextureRect( sf::IntRect( 0, 0, 16, 16 ) );// hud=blue (same color with 'empty' cells but different logical meaning)
		} else if ( m_cellDetailsDatabase.at( n ).cellCurrentContent == 6 ) {
			m_cellSpriteDatabase [ n ].setTextureRect( sf::IntRect( 80, 0, 16, 16 ) );	// debugMarked=gray
		}
	}
}
